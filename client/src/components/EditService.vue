<template>
  <modal v-bind:is-showing="isShowing" title="Edit Service" success-button="Save" v-on:success="success" v-on:cancel="cancel">
...
export default class EditService extends Vue {
  ...
  success() {
    this.error = false;
    axios
      .put(APIConfig.buildUrl(`/services/${this.service.id}`), this.service, {
        headers: {
